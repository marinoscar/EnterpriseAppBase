...
Deploy-time secrets (`JWT_SECRET`, `GOOGLE_CLIENT_SECRET`, etc.) live in the environment and are covered by the configuration reference above. `AWS_SECRET_ACCESS_KEY` is also deploy-time, but is SES-only since epic #372 — see the note below. This section covers a different category: secrets configured at runtime through the application rather than the environment, which cannot come from an env var because changing one requires a redeploy. As of issue #387 there are **two tables** under this heading, sharing one master key but owned at different levels:

- **`credentials`** (Prisma model `Credential`) — an **administrator** configures these once for the whole deployment, and every user shares them: SMTP (`smtp`, issue #109), Web Push's VAPID private key (`push_vapid`, issue #355), and the object-storage secret access key (`storage`, epic #372 — see [`docs/specs/storage-providers.md`](specs/storage-providers.md)).
- **`user_credentials`** (Prisma model `UserCredential`, issue #387) — **one user** supplies these for themselves, and nobody else — including an administrator — reads or writes another user's row. This is the foundation for a bring-your-own-key pattern (a personal `llm/anthropic` key that bills the user's own account instead of the deployment's shared one); as of this writing no feature consumes it yet, its registry of purposes ships empty, and it has no HTTP surface at all. See [`docs/specs/user-credentials.md`](specs/user-credentials.md) for the full design, including why this is a second table rather than a nullable owner column on `credentials`.

(Epic #345's node-broker job credentials are a *different* mechanism, deliberately: they are short-lived database roles minted directly at the database, never persisted as ciphertext anywhere — `job_node_secrets` records only a handle, never material. See `docs/specs/job-queue.md` §7.10.)

Both tables are managed exclusively by their own service — `CredentialsService` (`apps/api/src/credentials/credentials.service.ts`) for `credentials`, `UserCredentialsService` (`apps/api/src/credentials/user-credentials.service.ts`) for `user_credentials` — and **neither has an HTTP controller of its own**. Each system-store consumer above exposes its own admin surface (`EmailSettingsController`, `PushConfigController`, `StorageConfigController`) that calls `describe`/`setSecret`/`getSecret` directly, rather than either service ever accepting a raw `(purpose, name, secret)` triple from HTTP. That is deliberate: a generic "write any secret to any purpose" endpoint would let whichever permission guards it write into an address a completely different feature reads from — and for the per-user store specifically, the eventual HTTP surface has a stricter requirement still: every route must scope to the *authenticated caller's own* user id, never one taken from a path parameter or a request body.
...
**The per-user store (issue #387) derives one level further.** A `user_credentials` row is encrypted under `userCredentialPurpose(userId, purpose)`, which folds the owner into the domain rather than adding a parameter to the cipher itself:

```
domain    = "user:" + lowercase(userId) + ":" + purpose
derivedKey = HMAC-SHA256(masterKey, "enterpriseappbase:secret-cipher:v1:" + domain)
```

**Why**: without an owner in the domain, every user's personal credential for a given purpose would sit under one shared sub-key — the same key an equivalent system-wide credential of that purpose would use. An attacker with a SQL *write* but not the encryption key (a compromised admin console, an injection reaching `user_credentials`, a restored backup merged in wrong) could copy user A's ciphertext into user B's row; it would decrypt cleanly, because it genuinely is a valid ciphertext for that purpose, and the application would spend A's key as though it were B's — A's budget, A's rate limit, A's provider-side audit trail, driven by B. Folding the owner into the domain turns that same write into a blob encrypted under A's sub-key being opened with B's: GCM authentication fails, and the row is unreadable rather than quietly wrong. It is the identical lateral-movement argument the paragraph above makes between *purposes* ('smtp' vs 'storage'), applied one level down, between *owners* inside one purpose.

Two grammar rules make the collision proof hold, both enforced at runtime rather than assumed: `userId` must match a canonical UUID pattern and is then **lowercased** before the domain is built (PostgreSQL's `uuid` type compares case-insensitively, so an uppercase id still finds the right row — but without normalisation the cipher would derive a *different* key for it than the canonical spelling, and the row would be found and then fail to decrypt, reporting a rotation or tampering incident that never happened); and `purpose` may not contain a colon (`assertCredentialIdentifier` in `apps/api/src/credentials/credential-internals.ts`), so a user domain always carries exactly two colons after the fixed label prefix while a system domain's entire variable part carries none — which is what keeps the two domains provably disjoint. See [`docs/specs/user-credentials.md`](specs/user-credentials.md) §3 for the full proof, written out for a reader who does not want to open the source.
...
The decision table is deliberately not a simple "fails if missing" check — strictness is gated on whether the credential store is actually in use, and since issue #387 "the credential store" means **both** tables encrypted under this key:

| Key state | Rows in `credentials` **or** `user_credentials` | Behaviour |
|---|---|---|
...
| Absent (unset or empty string) | ≥ 1 row, in either table (or both) | **Throws** — boot fails, in every environment, including development |
| Absent | 0 rows, in both tables | **Warns** and boots normally |
| (any) | Either table's count fails — DB unreachable, or that table not yet migrated — including the **mixed** case where the other table's count succeeded and reported rows | **Warns** (naming which table(s) could not be probed) and boots normally — this is deliberately not reported as an encryption-key problem |
...
- **"Absent, rows exist" is fatal in every environment on purpose, and now covers both tables for the same reason.** Rows in `credentials` can only exist because `CredentialsService.setSecret` successfully called `encryptSecret`, and rows in `user_credentials` can only exist because `UserCredentialsService.setSecret` reaches that identical `encryptSecret` call — either one throws without a valid key. So a row in *either* table is equally proof that a key was working here at some point, and its current absence is a regression, not first-time setup. Counting only `credentials` (as this check did before issue #387) would put a deployment with zero deployment-wide credentials and any number of stored user credentials into the warn-and-boot branch below — booting clean while every user's personal key sits unreadable, a failure mode that would first surface as individual users separately reporting that their own key "stopped working," and get diagnosed as a per-user problem long before anyone connected it to a deployment-wide key regression.
- **The fatal message breaks its total out per table**, and separately names how many affected rows are user-owned, because the two halves have materially different recoveries: an administrator can re-enter a deployment-wide credential (SMTP, storage, VAPID) from the admin UI, but a user-owned credential can only ever be re-entered by its own owner — nobody else is permitted to write that row (see [`docs/specs/user-credentials.md`](specs/user-credentials.md)). "0 deployment-wide, 340 user-owned" and "340 deployment-wide, 0 user-owned" are the same total count and two very different mornings for whoever is paged. A per-table count is still a count, never an address — it names a table, not a `purpose`/`name` pair, so the existing no-addresses rule below is unaffected.
- **"Absent, zero rows in both tables" warns and boots** rather than refusing to start, because this check's decision is about whether the *stored ciphertext this process already has* can be read, not about whether the credential store will ever be used — a fresh install legitimately has zero rows in either table before its first login even completes. This still matters in practice: `infra/compose/.env.example` ships `SECRETS_ENCRYPTION_KEY=` uncommented but empty, and since epic #372 the credential store has a consumer (the object-storage secret access key) that almost every real deployment needs working from its first upload — see the note on `AWS_SECRET_ACCESS_KEY` above. A deployment that never fills this key in stays bootable, but storage, SMTP and Web Push all stay unusable until it does.
- **Both tables are probed together, with `Promise.allSettled`, not `Promise.all`.** `Promise.all` would reject with whichever query failed first and discard the other outcome entirely, so the boot-time warning could never say *which* table it could not read. That distinction matters more here than in an ordinary paired read: `user_credentials` is the newer of the two tables, so it is precisely the one missing on a deployment that has applied every migration up to but not including #387's — the warning names each table's probe outcome individually so that case reads as "the newer table isn't there yet," not as an undifferentiated database problem.
- **A failure on *either* probe lands in warn-and-boot, including the mixed case** — one table's count succeeds and reports rows, the other's count throws. This is deliberate, and it is the pre-#387-migration upgrade path: a deployment that already has `credentials` rows and has not yet applied the `user_credentials` migration must still be able to boot, because in some deployment setups it is that very container which then runs the migration. Throwing on the half that *could* be read would wedge exactly the upgrade this check must not block. Failing open here stays safe rather than merely convenient, because it cannot hide the dangerous state: if credentials genuinely exist unprotected, this same check throws on the very next boot that can read both tables.
- **The check does not attempt to decrypt any row, in either table.** It only counts rows. A key that is present and well-formed but is the *wrong* key for existing rows (e.g. a partially completed rotation) still passes startup validation cleanly — the failure only surfaces later, when `CredentialsService.getSecret` or `UserCredentialsService.getSecret` actually tries to decrypt that row. See the rotation runbook, [`docs/runbooks/rotate-secrets-encryption-key.md`](runbooks/rotate-secrets-encryption-key.md), for the operational implications.
...
**The per-user store follows the identical discipline, field for field.** `UserCredentialsService.describe(userId, purpose, name)` and `.list(userId, purpose?)` select exactly `{ userId, purpose, name, hint, label, createdAt, updatedAt }` — via `USER_CREDENTIAL_INFO_SELECT`, typed as `Record<keyof UserCredentialInfo, true>` so a select is checked against the response type at compile time in both directions: a field added to the response type without being selected fails to compile, and selecting `secret` fails too, because `UserCredentialInfo` has no such key and the object literal is checked for excess properties. `getSecret(userId, purpose, name)` is the sole plaintext-returning method, is never reachable from a controller (there is no controller in this module at all — see [`docs/specs/user-credentials.md`](specs/user-credentials.md) §7), and throws rather than returning `null` on a decrypt failure, for a reason stated more strongly than the system store's own: at the resolver level (`UserCredentialResolver.resolve`) that throw must be allowed to propagate uncaught, because catching it and falling back to the system credential would not merely fail silently — it would *succeed*, spending the deployment's shared quota while the user believes they are still on their own key. See [`docs/specs/user-credentials.md`](specs/user-credentials.md) §5.1 for the full argument.
